...
async function overview(req, res, next) {
  try {
    const [total, approved, pending, archived, stats, statusRows] = await Promise.all([
      ...
      Knowledge.countDocuments({ status: "approved" }),
      Knowledge.countDocuments({ status: "pending" }),
      Knowledge.countDocuments({ status: "archived" }),
      Knowledge.aggregate([
        { $group: { _id: null, totalViews: { $sum: "$viewCount" }, averageRating: { $avg: "$averageRating" } } }
      ]),
      Knowledge.aggregate([{ $group: { _id: "$status", count: { $sum: 1 } } }])
    ]);
...
async function hotKnowledge(req, res, next) {
  try {
    const items = await Knowledge.find({ status: "approved" })

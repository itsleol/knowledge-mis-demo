...
async function synchronizeKnowledgeCounters() {
  const ratingRows = await Feedback.aggregate([
    { $group: { _id: "$knowledgeId", averageRating: { $avg: "$rating" } } }
  ]);
  const favoriteRows = await Favorite.aggregate([
    { $group: { _id: "$knowledgeId", favoriteCount: { $sum: 1 } } }
  ]);

  const ratingMap = new Map(ratingRows.map((row) => [String(row._id), Number(row.averageRating.toFixed(1))]));
  const favoriteMap = new Map(favoriteRows.map((row) => [String(row._id), row.favoriteCount]));
  const knowledge = await Knowledge.find({}, "_id");
  if (!knowledge.length) return;

  await Knowledge.bulkWrite(knowledge.map((item) => ({
    updateOne: {
      filter: { _id: item._id },
      update: {
        $set: {
          averageRating: ratingMap.get(String(item._id)) || 0,
          favoriteCount: favoriteMap.get(String(item._id)) || 0
        }
      }
    }
  })));
...
async function seedDemoData({ reset = false } = {}) {
  ...
  const knowledge = await Knowledge.insertMany(
    rows.map(([knowledgeCode, title, summary, content, category, tags, creator, department, status, viewCount], index) => ({
      knowledgeCode,
      title,
      summary,
      content,
